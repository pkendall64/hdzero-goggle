...
void show_module_settings() {
    ...
    case MODULE_RAPIDFIRE:
        btn_group_show(&btn_group_rapidfire, true);
        btn_group_show(&btn_group_steadyview, false);
    ...
    case MODULE_STEADYVIEW_X:
        btn_group_show(&btn_group_rapidfire, false);
        //steadyview need repower to switch mode, old HW revision can not power cycle analog module
        btn_group_show(&btn_group_steadyview, getHwRevision() >= HW_REV_2 || btn_group_get_sel(&btn_group_module_type) == MODULE_STEADYVIEW_X);
        lv_obj_add_flag(back_btn2, LV_OBJ_FLAG_HIDDEN);
