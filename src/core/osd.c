...
void osd_channel_show(bool bShow) {
    ...
    if (channel_osd_mode & 0x80) {
        ...
        char *str = g_source_info.source == SOURCE_HDZERO ? channel2str(g_setting.source.hdzero_band, ch) : module_channel_str(ch-1);
        color = lv_color_make(0xFF, 0x20, 0x20);
        snprintf(buf, sizeof(buf), "  To %s?  ", str);
        ...
        snprintf(buf, sizeof(buf), "CH:%s", str);
